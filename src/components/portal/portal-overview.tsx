...
import { PortalFinanceSnapshot } from "./portal-finance-snapshot";
...
export function PortalOverview({ profile }: PortalOverviewProps) {
  return (
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(0,0.9fr)] lg:grid-rows-[auto_auto_auto] lg:items-start">
      <div className="order-2 min-w-0 lg:order-none lg:col-start-1 lg:row-start-1">
        <PortalActivityFeed items={profile.activity} />
      </div>
      <div className="order-3 min-w-0 lg:order-none lg:col-start-1 lg:row-start-2 lg:row-span-2">
      ...
      <div className="order-5 min-w-0 lg:order-none lg:col-start-2 lg:row-start-3">
        <PortalFinanceSnapshot summary={profile.finance.summary} />
      </div>
    </div>
  );
}
